...
链上 RPC 校验使用的订单快照必须与最终认领绑定到同一组支付条款。`ClaimPaymentConfirmation` 会在认领事务中重新读取订单，并比较支付方式、币种、法币、汇率、应付金额、收款地址、锁定地址标记和失效时间；用户在 RPC 请求期间重选支付方式后，旧交易不能写入新支付条款的订单。订单重选、取消、过期、确认成功和确认失败也只能通过带期望前置状态的字段级条件更新落库：分别是 `waiting` 到重选/取消/过期，以及 `confirming` 到成功/失败。条件不满足时不得覆写 `RefHash`、付款地址、确认时间或状态，且不得发送对应的成功/失败通知或商户回调。
